...
import { recommendDailyProblems, getDifficultyColor, getKnowledgePointName } from '@/utils/analysis';
import { Link } from 'react-router-dom';

export const DailyPractice = () => {
  const { userData } = useUserStore();
  const recommendedProblems = recommendDailyProblems(userData);
  ...
  const completedToday = userData.completedProblems.filter(
    cp => cp.completedAt.startsWith(new Date().toISOString().split('T')[0])
  );
    ...
    >
      ...
      <Card className="p-4 md:p-6 mb-4 md:mb-6 bg-gradient-to-r from-primary-600 to-accent-500 text-white">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-white/80">完成{recommendedProblems.length}道推荐题目，巩固薄弱知识点</p>
          </div>
          <div className="text-right">
            <p className="text-3xl font-bold">{completedToday.length}/{recommendedProblems.length}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {recommendedProblems.map((problem, index) => (
          ...
          >
            <Card hover className="p-4 md:p-5 cursor-pointer">
